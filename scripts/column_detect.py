...
2. **优先找整版栏目刊头**：版面上部/角落带"第X期"或"总第X期"字样、红底白字等醒目样式的刊头（如"气象博观 2026第30期 总第73期"）是**整版栏目条**——它管辖全版，所有文章都属于它。若找到刊头，直接输出"该刊头栏目名覆盖本版全部文章"，不要再把版内小标签当栏目。
3. **版内小色块/小标签不是栏目条**：版面内黄底/浅色小方块上的文字（如节气专题里的"气象解码""顺时而养""风物神州"、系列报道的分主题小标）是装饰性标签或专题内小栏目，忽略，不当作栏目。
4. 策划版/科普专题版面（整版同题）大概率有整版刊头，优先寻找。
5. 不要把文章标题、引题、副标题、导读、报头、刊名当作栏目条。
6. 每篇文章标题请尽量完整读取（至少15字）。
