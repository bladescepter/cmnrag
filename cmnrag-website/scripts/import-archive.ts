...
async function main() {
	...
	const existing = new Map(
		(await query("SELECT source_path, source_sha256 FROM articles", [])).map((row) => [String(row.source_path), String(row.source_sha256)]),
	);
	...
	console.log(JSON.stringify({ runId, sourceRoot, articles: inserted, changed, skipped, searchableArticles: searchableArticles.length, metadataOnlyArticles: metadataOnlyCount }));
}
